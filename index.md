...
중요한점: 예측 값 y 벡터는 class갯수 K의 크기를 갖는 벡터다.  $K \time 1$ 의 크기를 갖는다. 한 벡터는 위 그림처럼 최종 분류된 하나의 class만 1이고 나머지는 0으로 구성된다. 
...
## Predicting Movie Ratings: Problem define
---
...
## Predicting Movie Ratings: Practice
---
...
Guese $\theta$ -> $x$ -> $\theta$ -> $x$ -> $\theta$ -> $x$ ...

random 하게 $\theta$ 를 구해본 뒤 이것을 가지고 $x$ 를 구한다. 그뒤 그 $x$ 를 가지고 다시 조금 더 정확한 $\theta$ 를 구한다. 또다시 그 $\theta$ 를 가지고 보다 더 정확한 $x$ 를 구해본다. 이것을 반복하다보면 결국 매우 훌륭한 $x$ 와 $\theta$ 를 얻을 수 있다. 이것이 Collaborative Filtering 알고리즘이다.  
